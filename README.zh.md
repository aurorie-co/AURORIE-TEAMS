...
推荐入口是 `orchestrator` agent。它读取 `.claude/routing.json`，按 `positive_keywords`（+1 分/命中）和 `negative_keywords`（−2 分/命中）对每个团队规则打分，选出最匹配的团队后派发 Lead。当请求夹杂多个信号时，会识别*主意图*以避免歧义分发。
...
- **路由规则：** 编辑 `.claude/routing.json` 控制请求与团队的映射关系。v2 schema 每条规则支持 `positive_keywords`（+1 分）、`negative_keywords`（−2 分，强排除信号）和 `example_requests`（用于同分时决胜）
