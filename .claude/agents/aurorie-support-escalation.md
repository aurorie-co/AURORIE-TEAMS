...
1. Read the ticket and `input_context`. If a `triage-report.md` artifact is referenced, read it in full.
   If no `triage-report.md` artifact is present in `input_context`, return `FAILED: triage report is required before escalation can be planned. Dispatch aurorie-support-triage first and pass the result as artifact.`
